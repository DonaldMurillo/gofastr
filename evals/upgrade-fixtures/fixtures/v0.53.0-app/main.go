...
import (
	...
	"example.com/upgrade-fixture/entities"
)
...
func main() {
	...
	fwApp.Mount(uihost.New(site, uihost.WithCustomCSS(fontFaceCSS+appBaseCSS()), uihost.WithDescription("UpgradeFixture — manage users, tasks, and tags."), uihost.WithAppIcon(appIconPNG()), uihost.WithSitemap(uihost.SitemapConfig{BaseURL: appBaseURL()}), uihost.WithRobots(uihost.RobotsConfig{Disallow: []string{"/__gofastr/"}}), uihost.WithStrict()))
...
func openDB(runtimeIsolation *isolation.Runtime) (*sql.DB, error) {
	...
	dsn := getEnv("DATABASE_URL", "file:fixture.db")
...
func appIconPNG() []byte {
	img, err := fwimage.NewGradient(512, 512, "#4338CA", "#241E6F")
	if err != nil {
