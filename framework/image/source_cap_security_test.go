...
// Pins (2026-09-06/07 adversarial round 5, phase 2; family enumeration;
// tier T3). CONTRACT-QUESTION resolved 2026-09-07: yes — the package caps
// source bytes too (64 MiB default, Config.MaxSourceBytes).
// untrusted image bytes decode through a source-byte cap —
...
// Fix: DecodeWithConfig reads through io.LimitReader(r, MaxSourceBytes+1)
// and returns ErrSourceTooLarge past the cap, so the pixel guard is
// reachable before the buffer is.
...
func TestDecodeRedCapsSourceBytes(t *testing.T) {
	...
	// Bounded cleanup: cancel the source and join the goroutine. When
	// Decode already returned (the capped read ends at limit+1 bytes,
	// before the threshold), checkDone consumed the channel and joining
	// again would falsely time out.
	if !returned {
		r.cancel()
		select {
		case <-done:
