...
// enumeration; tier T2). Pinned siblings: TestSEO_BundleURLsRejectUnsafeSchemes
...
// redJSONLDDangerousURLs is the scheme-hostile subset of dangerousURLs
...
var redJSONLDDangerousURLs = []struct{ name, url, marker string }{
...
// redHostileArticle builds an Article carrying the hostile URL in both of its
// URL-typed fields.
func redHostileArticle(u string) seo.Article {
	art := seo.NewArticle()
...
// TestSEOJSONLDRedSchemeDropped: hostile schemes in Schema.org URL fields must
// not reach the emitted ld+json, neither at the seo.Render level nor through
// the served page head.
func TestSEOJSONLDRedSchemeDropped(t *testing.T) {
	// Leg A — package-emitter level: seo.Render output.
	for _, u := range redJSONLDDangerousURLs {
		t.Run("render/"+u.name, func(t *testing.T) {
			out := string(seo.Render(redHostileArticle(u.url)))
	...
	for _, u := range redJSONLDDangerousURLs {
		t.Run("page/"+u.name, func(t *testing.T) {
			page := renderBundleSEOPage(t, SEO{Schema: []seo.Thing{redHostileArticle(u.url)}})
