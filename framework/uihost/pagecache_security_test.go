...
// Pins: a live-session page 200 carries Cache-Control: no-store (+ Vary:
// Cookie) before the live/dead split in handlePage, exactly like
// handlePartialPage — the page body is per-user rendered content.
...
func TestPageLiveSessionNoStore(t *testing.T) {
	ds := actionsHost()
