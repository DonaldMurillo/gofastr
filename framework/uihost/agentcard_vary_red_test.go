...
// CONTRACT-QUESTION red: agentready.go documents no Vary promise, but resolveBaseURL's
// own comment and the wellknown.go sibling contract state the cache-poisoning rationale
// for exactly this dataflow, and the signed-card variant already panics unless BaseURL
// is pinned — this asserts the unsigned remainder carries Vary; if refusing unpinned
// base URLs is the chosen fix instead, delete this and pin that.
...
// agentCardRedVaryFields collects Vary field names across header lines and
...
func agentCardRedVaryFields(rec *httptest.ResponseRecorder) map[string]bool {
...
// TestAgentCardRedVaryOnOriginInputs: the unsigned A2A card at both mounted
...
// agent's card.
func TestAgentCardRedVaryOnOriginInputs(t *testing.T) {
	...
	for _, path := range []string{"/.well-known/agent-card.json", "/.well-known/agent.json"} {
		...
		have := agentCardRedVaryFields(rec)
...
// TestAgentLinkHeadersRedVaryOrigin: HTML pages whose Link headers embed
...
func TestAgentLinkHeadersRedVaryOrigin(t *testing.T) {
	ds := newISOHost(
