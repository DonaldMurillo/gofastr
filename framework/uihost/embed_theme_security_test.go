...
// Pins: the customer-supplied ?theme= token payload decodes through
// handler.UnmarshalStrict — repeated or case-folded keys refuse, and a
// refusal degrades to the app theme (ok=false), never a
// silently-normalized variant.
...
func TestResolveEmbedThemeRejectsDupKeys(t *testing.T) {
