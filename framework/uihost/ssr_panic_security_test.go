...
// Pins: every host-supplied screen Render/RenderCtx, Load, and
// post-Load ScreenTitle/ScreenLang re-read runs under containment — a
// panicking hook renders the error path, never escapes the request.
// (Fix landed in core-ui/app: renderComponentAs through SafeRenderCtx,
// recover around Load and the post-Load metadata re-reads.)
//
...
// ssrGetNoPanic mirrors serveExpectNoPanic (uihost_security_test.go)
...
func ssrGetNoPanic(t *testing.T, tag string, ds *UIHost, path string, hdr map[string]string) *httptest.ResponseRecorder {
	t.Helper()
...
// paramPanicScreen: panicScreen's twin for dynamic routes — the
...
type paramPanicScreen struct{}

func (paramPanicScreen) Render() render.HTML         { panic("test: screen render boom") }
func (paramPanicScreen) SetParams(map[string]string) {}

// TestSSRRenderPanicContained: a panicking screen component must not
...
func TestSSRRenderPanicContained(t *testing.T) {
	...
	ssrGetNoPanic(t, "ssr-rendercomponent-unnetted", New(a), "/", nil)
	...
	aIx.Register("/products/:id", paramPanicScreen{}, nil,
		app.InterceptFrom("/products", app.ScreenDrawer))
	ssrGetNoPanic(t, "ssr-rendercomponent-unnetted", New(aIx), "/products/7", map[string]string{
...
// loadBoom: Load panics, Render is healthy — the panic can only come
// from the loader hook.
type loadBoom struct{}

func (c *loadBoom) Load(context.Context) error { panic("test: screen load boom") }
func (c *loadBoom) Render() render.HTML        { return render.Text("loaded") }

// loadErr: the control twin — Load fails through the error channel.
type loadErr struct{}

func (c *loadErr) Load(context.Context) error { return context.DeadlineExceeded }
func (c *loadErr) Render() render.HTML        { return render.Text("loaded") }

// titleBoom: ScreenTitle works on the 1st call (the registration-time
...
type titleBoom struct{ calls int }

func (c *titleBoom) Render() render.HTML { return render.Text("titled") }
func (c *titleBoom) ScreenTitle() string {
	c.calls++
	if c.calls >= 2 {
...
// TestSSRLoadPanicContained: the pre-render hooks on the SSR pipeline
...
func TestSSRLoadPanicContained(t *testing.T) {
	...
	func() {
		...
		aT.Register("/", &titleBoom{}, nil)
	}()
	ssrGetNoPanic(t, "ssr-loadhook-unnetted", New(aT), "/", nil)
}
