...
// enumeration; tier T2, low). Tests-only; no fix applied.
//
...
//
// Fix direction: run the decoded token through the any-depth strict-key walk
// the crud body path already uses (handler.CheckObjectKeys — exact and
// case-folded duplicates), keeping the unknown-field tolerance and the
// pinned 16 KiB / 64-field caps; EncodeCursor output is single-valued per
// key, so round-trips stay green.
