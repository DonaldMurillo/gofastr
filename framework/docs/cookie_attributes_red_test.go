...
// hunts.
// Surfaces: framework/docs/content/i18n.md:174-184 — the canonical
// "change language" handler sets only Name/Value/Path from raw
// r.FormValue("lang"), in the doc that two paragraphs earlier calls
// resolver values "attacker-controlled" (i18n.md:154).
// Finding: apps copying the canonical locale handler mint an attribute-less
// cookie — readable by any injected script (no HttpOnly), traveling over
// plain HTTP after a downgrade (no Secure), riding cross-site requests
// (no SameSite). i18n.md:177 fails all three keys today.
...
// TestDocsCookieRedAttributesTaught runs the GOFASTR1404 check over every
...
// have flagged.
func TestDocsCookieRedAttributesTaught(t *testing.T) {
