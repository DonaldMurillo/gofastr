...
| `data-fui-disclosure` | Legacy spelling, read by the KERNEL only: the navigator closes such a `<details>` on a client-side navigation and on an in-panel anchor click (`html.Details{Disclosure: true}` emits it). The full disclosure behaviour — the aria-expanded mirror, Escape one level at a time with focus return, the trap posture — belongs to `headless.Disclosure`'s `data-hui-disclosure` hook, bound by the registered `headless-disclosure` module (framework/headless). |
...
| `data-fui-disclosure-persist` | Kernel close-exemption for a `data-fui-disclosure` `<details>`, and a deprecated one (removed in the release after this one; CHANGELOG, Unreleased "Deprecated"): the navigator leaves it open across a client-side navigation. The headless anatomy's equivalent carries a storage key (`data-hui-disclosure-persist="<key>"`, session-restored by the registered `headless-disclosure` module). |
...
| `data-fui-toast-stack="<name>"` | Marks the container into which `__gofastr.toast()` appends items. The name matches the widget name passed to `preset.ToastStack`; `headless-feedback` scans both this and the `data-hui-toast-stack` spelling. |
...
| `data-hui-panehost` | Emitted by `framework/ui.PaneHost` on its root `<div>` (alongside `data-fui-comp="ui-pane-host"`). Marker the registered `headless-panehost` module scans for to wire open/close/swap triggers and the responsive overlay-drawer collapse. |
...
| `data-fui-pane-deeplink` | The legacy spelling of the pane deep-link declaration, still read by the kernel's stateful-params scan (nav.js `_statefulParams`) so a history move across a pane param from an old host stays stateful; no handler owns it and no component emits it anymore — `PaneHost` writes `data-hui-pane-deeplink`. |
...
inside a `[data-fui-scrollspy]` wrapper (the legacy spelling of the
scroll-spy rail, kept for the hands-off rule; today's rail is
`data-hui-rail`, whose links carry `.is-active`, never `activelink`'s
...
opts a link into segment-prefix matching: `/docs` lights up on `/docs`
and `/docs/getting-started`, never on `/docs-old`.
