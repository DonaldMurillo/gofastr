...
| `data-fui-tabs-vacate` | On a `framework/ui.Tabs` wrapper (`TabsConfig.VacateHidden`): hidden panels ship empty (content in `data-fui-tabs-stash`); the `tabs` module restores content on first show and moves live nodes out/in on later switches, so swapped island content survives re-show. Vacated panels are detached: document-scoped updates targeting them are dropped until re-show. |
