...
| Browser-owned state without an account | `framework/local`: `local.Define` collections, `SeedSignal`, `Send`/`Upload.Wrap`, `Put`, `Mirror` | The browser is the store (IndexedDB through the kernel's `local` primitive), the declaration is Go, and every bridge to the server is explicit: a seeded signal, an upload the trigger names, a record the response writes, a cookie-sized mirror the server reads at first paint. Best-effort, size-capped, versioned; not offline sync. | [`/forms/draft-notes`](../../../examples/site/screen_localnotes.go) | [Local state](local-state.md), [Signal store](signal-store.md) |
...
| Persisted slice vs local store | One projection the browser should remember, with no server bridge. Use `store.Persist`. | Named collections with caps and migrations, a filtered list, or a value a screen reads at render or action time or pushes back. Use `framework/local`. |
...
- An **affinity-bound island** holds mutable widget/island objects in one
  ...
  Redesign the handler around reconstructable state, reading from
...
| Polling | The browser re-fetches a region on a Go-duration interval; any replica answers from the DB | Passive freshness for counters, statuses, and dashboards without a held connection | Surfaces that need push under a second, or that must reflect the connection itself (presence) |
...
- [Optimistic UI](optimistic-ui.md) for the mutation lifecycle contract and
  the seven composed recipes (toggle, inline edit, create, delete, kanban,
  group mutex, slow/failure).
...
- **Calling a signal durable state.** It is a UI projection. Persist business
  ...
  projection, and `framework/local` a declared set of records; both are
  best-effort by contract and a user can clear them between two page loads.
