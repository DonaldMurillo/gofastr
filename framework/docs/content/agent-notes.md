...
## 2026-07-13 - natural UI guidance and framework-snapshot evaluation

- **Scope:** `cmd/gofastr/init.go`, generated `AGENTS.md` / `CLAUDE.md` / host skill, `framework/ui/agents.md`, UI composition docs, and `evals/ui-quality`.
- **Symptom:** the UI-quality experiment was measuring evaluator-authored design overrides rather than GoFastr's natural behavior. Candidate `AGENTS.md` files replaced the scaffold's guidance, Codex builders ran with repository rules disabled, and the strongest historical result used 661 lines of app-owned CSS. The scaffold itself reinforced that drift by generating `screens/styles.go`, mounting `WithCustomCSS`, and telling users to customize local CSS despite the repository's one-styling-surface rule.
- **Change:** `gofastr init` now emits an app-owned `DESIGN.md`, a framework-composed home screen, and zero app CSS; reinit creates a missing design file but never overwrites product decisions. Generated guidance requires product intent, composition selection, mobile prioritization, and pixel review before component selection. The embedded `ui-composition-recipes` topic maps common product shapes to existing framework primitives. The evaluator now varies framework roots, preserves and fingerprints untouched generated onboarding, uses neutral product briefs, keeps builder repository rules enabled, supports Codex, OMP / GLM-5.2, and Claude Code / Opus builders (Codex or Claude for image judging), freezes transitions before forcing a capture scheme, and verifies the HTML theme attribute and browser color-scheme meta agree.
- **Next time:** treat changes to scaffold guidance, embedded docs, and UI primitives as one framework treatment. Never inject scenario-specific design prescriptions into a candidate and then describe the score as framework quality. A one-snapshot run can certify the absolute bar, but a winner requires at least two eligible snapshots.
...
## 2026-07-13 - natural-operational-composition

- Scope: `framework/ui`, `cmd/gofastr`, `framework/docs/content`, `examples/site`
- Symptom: a neutral generated-app evaluation correctly found the existing primitives but still composed a generic Box/Stack hero, stretched its CTA, repeated incident state in a Banner, and wrapped a long SiteHeader identity off-screen on mobile. A design-override treatment showed a stronger shape, but its score was causally invalid because the prompt supplied the design decisions.
- Evidence: `ui.RecordSummary`, `ui.MetricBand`, and `SiteHeaderConfig.MobileBrand` move the reusable composition, signal density, action sizing, and mobile identity behavior into the framework; the ordinary host skill, generated DESIGN checklist, composition recipe, live catalog, and component tests teach and enforce the same contract without injecting a product-specific design override. The 390px rendered gate also exposed and fixed `DocLayout` retaining a 953px flex-item min-content width, while the all-pages axe pass exposed and fixed undersized `ValidationSummary` links. A fresh neutral candidate then exposed another default-path gap: it correctly selected `ThemeToggle`, but the scaffold mounted no adaptive theme, leaving dark UA link colors on light surfaces. Fresh scaffolds now mount `framework/ui/theme.Default()`, `gofastr theme init` emits a complete dark palette, linked SiteHeader brands have framework-owned styling, and visible body/footer links are explicitly directed to `ui.Link` / `ui.SiteFooter`. The next neutral OMP build passed every technical check and scored 8.255 desktop / 8.595 mobile with Claude Code judges, confirming natural transfer. Its remaining screenshots exposed two reusable defaults: an undefined `AvatarGroup` overflow background in dark mode and a `RecordSummary` primary action that arrived too late on phones while wide summaries lacked a bounded supporting rail. `AvatarGroup` now uses `--color-surface-soft`; `RecordSummary.Aside` supplies that rail and the `Actions` slot now lives in the lead region, with concise-summary and metric-hint guidance synchronized across generated artifacts.
- Follow-up: a bounded neutral run completed in 16.2 builder minutes, technically passed, and scored 8.185 desktop / 8.645 mobile. It proved the early action, supporting rail, metric hints, adaptive theme, mobile identity, and linked chrome all transferred naturally. Its consistent remaining defect was the framework's 40% avatar overlap clipping two-letter initials. The same run exposed that a normally exiting OMP process could orphan its local `go run` server. Windows builder and judge processes now execute inside kill-on-close jobs in addition to timeout tree cancellation.
- Second follow-up: a fresh neutral run completed in 11.1 builder minutes with no leaked server, passed every technical check, and scored 8.24 desktop / 7.75 mobile. Its different composition surfaced reusable narrow-screen defects rather than a reason to tune the candidate: a five-item `MetricBand` stranded an empty quadrant, action labels wrapped inside controls, and a small AvatarGroup still crowded initials with status dots. Odd bands now balance the final row, buttons wrap as whole controls, avatar overlap is 10% with smaller corner dots, and generated guidance explicitly pairs related bounded detail modules across a wide desktop canvas before reflowing them on phones.
- Third follow-up: the next neutral build finished just under the 30-minute ceiling but failed the technical gate because three short actions visually overflowed the phone despite `ClusterConfig{}`. The root cause was a zero-value API contradiction: `Cluster` was documented as wrapping by default while `Wrap bool` made the zero value emit `nowrap`. `Cluster` now truly wraps by default, `NoWrap` is the explicit opt-out, buttons keep their labels intact, and the rendered RecordSummary gate reproduces a nested three-action Cluster while asserting both document width and full-screenshot width.
- Next time: use override-heavy runs only as diagnostic oracles. Promote repeated useful behavior into named framework primitives and ordinary discoverability, then measure the improvement with a fresh neutral prompt against an immutable framework snapshot.
