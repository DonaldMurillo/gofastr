...
- Scope: `framework/image/`, `framework/docs/content/image.md`, `examples/website/screen_image_pipeline.go`, `examples/website/e2e_image_pipeline_test.go`
...
- Evidence: this commit adds `framework/image` (chain API: Resize / Rotate / Flip / Flop / Modulate / AutoOrient + JPEG/PNG/GIF/BMP/TIFF encoders + Placeholder + BlurHash) — pure Go, zero CGo, only `golang.org/x/image` as a dependency beyond stdlib. Minimal EXIF orientation parser inline. Decompression-bomb guard defaults to 268 MP (Bun parity). `/framework-ui/image-pipeline` demo screen renders every operation against a synthetic gradient with `data-test` selectors and chromedp E2E coverage in `examples/website/e2e_image_pipeline_test.go`. Lossless WebP / HEIC / AVIF return `ErrFormatUnsupported`; VP8L lossless encoder is planned as a follow-up under `framework/image/webp/`.
...
- Scope: `framework/image/internal/vp8l/`, `framework/image/`, `framework/ui/`, `examples/website/`, `framework/docs/content/image.md`
