...
├── gen/                # generated Go (regenerated by `gofastr generate --from`; gitignored)
...
**1. Declarations + generated code.** Entities are the canonical
...
generate from a blueprint, `gen/` is the Go it produces (`gofastr generate
--from=gofastr.yml`); read and debug `gen/`, but edit the *declaration*, not
the output — it's regenerated. Most small apps never need more than this.
