...
reorder the words but must keep the placeholders. Keys added for the
...
→ `ui.passwordInput.show`/`hide`, `Previous`/`Next` →
`ui.pagination.previous`/`next`, and the success/warning/danger tone
words → `ui.toast.success`/`warning`/`error`.
