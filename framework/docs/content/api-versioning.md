...
the OpenAPI document, and the MCP tools. A client, an SDK generator,
...
spec or the tool paths, so they can't drift from the routes.
...
validation knob that DOES reach the DDL, via `VARCHAR(n)`, so it is
...
- **At most one Seed.** Two versions may not both declare a `Seed`,
  because functions cannot be compared for equality, so the second is
  ambiguous.
...
(`/posts`, schema `posts`, tag `posts`), so nothing changes for existing
apps.
