...
session middleware. You'll want a signed-in user in the next
...
(including the auth setup); `main.go` is the entrypoint. Read them; they are
...
-->
```go
// about.go is your own file, package main, alongside the generated ones.
package main
...
Save and let the dev server rebuild, then verify the full model. Log in
