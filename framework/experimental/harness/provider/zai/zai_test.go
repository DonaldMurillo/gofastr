...
// message. Unlike OpenRouter, ZAI must NOT send HTTP-Referer or
// X-Title.
func TestChatRequestShape(t *testing.T) {
	...
	if gotReferer != "" {
		t.Errorf("HTTP-Referer = %q; ZAI must not send it", gotReferer)
	}
	if gotTitle != "" {
		t.Errorf("X-Title = %q; ZAI must not send it", gotTitle)
	}
	if body.Model != "m" {
