...
// Property: password redaction is total, not prefix-deep. In a URL the userinfo ends at
...
func TestRedactRedAtInPasswordTotallyMasked(t *testing.T) {
	...
	// Control: a password without '@' masks fully — the half that works
	// today and must keep working after the fix.
	if out := RedactDSN("postgres://admin:hunter2@db.example:5432/app"); !strings.Contains(out, ":****@db.example") || strings.Contains(out, "hunter2") {
