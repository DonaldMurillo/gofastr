...
func TestRecordSummaryRendersBoundedCompositionSlots(t *testing.T) {
	...
	for _, want := range []string{
		...
		`fui-record-summary__status`,
		`Next decision at 14:30`,
		`fui-record-summary__metrics`,
		`fui-record-summary__aside`,
		`Bridge roster`,
		`fui-record-summary__lead--with-support`,
		`Commander: Mina`,
		`fui-record-summary__actions`,
		`Open incident`,
	} {
...
func TestRecordSummaryDefaultsToH1AndNeutralTone(t *testing.T) {
	...
	if !strings.Contains(h, `<h1 class="fui-record-summary__title"`) || !strings.Contains(h, `>Overview</h1>`) {
		t.Fatalf("default heading must be h1:\n%s", h)
	}
	if classTokenPrefixPresent(h, "fui-record-summary--") {
...
func TestMetricBandRendersSemanticSignals(t *testing.T) {
	...
	for _, want := range []string{
		...
		`<dd class="fui-metric-band__value">32%</dd>`,
		`<dd class="fui-metric-band__hint">checkout requests</dd>`,
	} {
		if !strings.Contains(h, want) {
...
func TestMetricBandCSSIsFlatWideAndTwoColumnMobile(t *testing.T) {
	...
	for _, want := range []string{
		...
		`.fui-metric-band--3 .fui-metric-band__item:last-child`,
		`.fui-metric-band--5 .fui-metric-band__item:last-child`,
		`grid-column: 1 / -1`,
		`text-align: center`,
		...
		`:not(.fui-metric-band--1) .fui-metric-band__item:nth-child(odd)`,
	} {
		if !strings.Contains(css, want) {
