...
// real "fui-menu" component stylesheet with the registry, so the test
...
// pages this markup ships on always serve /__gofastr/comp/fui-menu.css.
...
func menuTriggerAxeServer(t *testing.T, body string) *httptest.Server {
	...
	if behaviors == nil {
		t.Fatal("no registered behaviours in this binary")
	}
...
<script type="application/json" id="gofastr-behaviors">%s</script>
</head><body>
...
</body></html>`, catJSON, behaviors, body)
	})
...
func renderTriggerMenu() string {
	return string(ui.Menu(ui.MenuConfig{
		...
		Items: []ui.MenuItem{
			...
			// The separator and the form-POST row make this scan the
			// one that proves the menu's owned-element contract: an
			// hr[role=separator] is an owned element, and the form
			// around the submit row must carry role="none" or the
			// panel fails aria-required-children.
			{Separator: true},
			{Label: "Stop impersonation", Action: &ui.MenuAction{
				Path:   "/admin/stop",
...
// once opened the rows are ordinary menu markup. Moved from the
// retired core-ui/runtime lazy suite.
func TestMenuLazyAxeClean(t *testing.T) {
	...
	for _, scheme := range axetest.Schemes {
		...
		if err := chromedp.Run(ctx,
			...
			cancel()
			t.Fatalf("axe setup closed (%s): %v", scheme, err)
		}
		vs, err := axetest.Scan(ctx, scheme, nil)
		cancel()
		if err != nil {
		...
		if err := chromedp.Run(ctx2,
			...
			chromedp.Sleep(700*time.Millisecond),
			axetest.Prepare(scheme),
			chromedp.Click(`button#lz-open`, chromedp.ByQuery),
			chromedp.Sleep(250*time.Millisecond),
		); err != nil {
			cancel2()
			t.Fatalf("axe open setup (%s): %v", scheme, err)
...
// renderLazyMenu renders a lazy trigger menu whose rows ship inside
// the inert template.
func renderLazyMenu() string {
	return string(ui.Menu(ui.MenuConfig{
		ID:             "lz",
		TriggerElement: render.HTML(`<button type="button" id="lz-open">Open lazy menu</button>`),
		LazyPanel:      true,
		Items: []ui.MenuItem{
			{Label: "Row one", Href: "/one"},
			{Label: "Row two", Href: "/two"},
		},
	}))
}

// renderCarousel renders a live Carousel fixture for the axe scan: a
// region + track + slides + dots + arrows, the exact anatomy the site
// serves on /components/carousel.
func renderCarousel() string {
	return string(ui.Carousel(ui.CarouselConfig{
		ID:    "axe-carousel",
		Label: "Featured work",
		Slides: []ui.CarouselSlide{
			{Content: render.HTML(`<p>Slide one</p>`)},
			{Content: render.HTML(`<p>Slide two</p>`)},
			{Content: render.HTML(`<p>Slide three</p>`)},
		},
	}))
}

// TestCarouselAxeClean: the carousel must scan clean under both colour
// schemes with the real component CSS applied. This is the seconds-scale
// proof of the allowed-role rule the site's eight-minute sweep caught:
// a ul/li track tripped aria-allowed-role (group on li) and the list
// rule (a ul of role=group children); the div track trips neither.
func TestCarouselAxeClean(t *testing.T) {
	srv := menuTriggerAxeServer(t, renderCarousel())
	browser := axetest.NewBrowser(t)
	for _, scheme := range axetest.Schemes {
		ctx, cancel := axetest.NewTab(t, browser)
		if err := chromedp.Run(ctx,
		...
		cancel()
		if err != nil {
			t.Fatalf("axe scan (%s): %v", scheme, err)
		}
		for _, v := range vs {
			t.Errorf("carousel (%s scheme): [%s] %s", scheme, v.ID, v.Help)
		}
	}
}
