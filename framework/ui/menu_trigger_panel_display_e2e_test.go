...
// the given data-hui-menu value.
func menuPanelDetailsOpen(dst *string, menu string) chromedp.Action {
	return chromedp.Evaluate(`String(document.querySelector('details[data-hui-menu="`+menu+`"]').open)`, dst)
}
...
func TestMenuTriggerPanelDisplay(t *testing.T) {
	...
	if err := chromedp.Run(ctx,
		...
		// /__gofastr/comp/fui-menu.css must both finish before any
	...
	// hiding (details[data-hui-menu]:not([open]) rule).
	var disp string
	...
	// must leave "none" (the .ffui-menu__panel rule sets grid).
	if err := chromedp.Run(ctx,
		chromedp.Evaluate(`document.querySelector('[data-hui-menu-trigger="um"] button').click()`, nil),
	...
	if err := chromedp.Run(ctx,
		chromedp.Evaluate(`document.querySelector('details[data-hui-menu="sm"] > summary.fui-menu__trigger').click()`, nil),
		chromedp.Sleep(250*time.Millisecond),
...
func TestMenuTriggerPanelSubmenuDisplay(t *testing.T) {
	...
	if err := chromedp.Run(ctx,
		chromedp.Evaluate(`document.querySelector('details[data-hui-menu="um-panel-sub-1"] > summary').click()`, nil),
		chromedp.Sleep(250*time.Millisecond),
