...
// Pins: a typed hook whose payload cannot be coerced to the declared shape fails closed —
// the callback is never invoked and an error names the drift.
// Surfaces: framework/typed_hooks.go::OnBeforeDelete, OnAfterDelete.
// Finding (2026-09-06 adversarial pass, round 5): both Delete wrappers did
// `id, _ := data.(string)`, so a drifted payload (anything but a string) was
// silently coerced to "" and the callback was STILL invoked, returning nil.
...
// Fix: type-assert strictly and return a naming error on drift, mirroring the
