...
// header pixel dimensions of an untrusted image must pass the
...
// Fix: stripPNG/stripJPEG run DecodeConfig + checkStripPixelCap (the
// pipeline DefaultMaxPixels value) before any pixel decode; the upload
// fails closed (the strip path was already fail-closed on structural
// errors).
