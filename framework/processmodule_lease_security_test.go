...
// Pins: a store unreachable past LeaseTTL must drain AND RECOVER when the store
...
// load.
// Fix: on the first successful refreshDesired after a lease-expired drain,
// clear leaseFailing and route the childless Ready slot through the spawn path;
// snapshot() copies leaseFailing and the proxy/tools gates read the real flag.
