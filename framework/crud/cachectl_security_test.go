...
// Pins (2026-09-06/07 adversarial round 5, phase 2; family enumeration; tier T1):
// a GET response whose rows are scoped to the caller (OwnerField/
...
// Fix: Cache-Control: no-store (admin-gate idiom) is stamped at the top of
// every CRUD handler (and writeJSONError) before the scope checks, so
// 401/403 arms carry it too; serveCursorList and ServeStreamingList stamp
// their own paths.
