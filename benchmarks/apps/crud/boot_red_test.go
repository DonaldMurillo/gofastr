...
// TestBootRedHealthzReachable builds the crud bench app, boots it on a
...
func TestBootRedHealthzReachable(t *testing.T) {
	...
	for {
		...
		select {
		case werr := <-exited:
			t.Errorf("SECURITY: [bench-healthz-boot] crud bench app exited before /healthz answered (wait=%v) — the manual GET /healthz (main.go:41) collides with the framework auto-mount (health.go:186 via app.go:3219) and the router panic kills every boot, bricking the resource-bench lane; stderr tail: %.400s", werr, stderr.String())
			return
