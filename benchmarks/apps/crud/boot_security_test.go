...
// Pins: a shipped bench app reaches the serving state. The app's own
// source registers GET /healthz "so the resource runner can wait for
// readiness" (main.go:41-44) — the resource-bench lane is dead if that
// probe can never answer.
//
// Surfaces: benchmarks/apps/crud/main.go:41-44 registers GET /healthz
...
// landed, unwired in CI so nothing noticed).
//
...
// TestBootHealthzReachable builds the crud bench app, boots it on a
// free port, and requires GET /healthz to answer 200 within a bounded
// deadline. Today the child dies at Start (duplicate /healthz route) and
// the probe never answers; the captured stderr tail names the conflict.
func TestBootHealthzReachable(t *testing.T) {
	if testing.Short() {
	...
	for {
		...
		select {
		case werr := <-exited:
			t.Errorf("SECURITY: [bench-healthz-boot] crud bench app exited before /healthz answered (wait=%v) — the manual GET /healthz collides with the framework auto-mount (health.go:186 via app.go:3219) and the router panic kills every boot, bricking the resource-bench lane; stderr tail: %.400s", werr, stderr.String())
			return
		case <-time.After(100 * time.Millisecond):
		}
		if time.Now().After(deadline) {
			t.Errorf("SECURITY: [bench-healthz-boot] crud bench app never answered GET /healthz within 10s (resource-runner readiness brick); stderr tail: %.400s", stderr.String())
			return
		}
	}
}
