...
10. **Read the PR's line-level review comments before merging.** After
    ...
    which returns only issue-level comments. Read `/pulls/<N>/reviews`
    too, and read the BODY, not just its "Actionable comments posted"
    count: findings the bot could not attach to a line sit there under
    "Outside diff range comments", and PR #206 hid five that way —
    including a Major on the change set's own anti-vacuity gate. A review bot showing `pass`
