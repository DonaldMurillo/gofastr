...
// Pins: every authenticated long-lived stream surface enforces the house
// per-principal seat cap — 16 by default, core/stream policy shape — on
// SubscribeToTask and SendStreamingMessage streams, freed when the relay
// loop returns (2026-09-06 adversarial pass, round 5).
...
func (st *a2aSeatStream) pump(body io.Reader) {
	...
	if err := sc.Err(); err != nil {
		// A read error (client reset mid-line) ends the drain the same
		// way EOF does; eof below is the observable either way.
		_ = err
	}
	st.markFirstData()
	close(st.eof)
