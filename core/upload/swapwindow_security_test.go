...
// TestLocalStorageRefusesMidResolutionSymlinkPlant pins the
// resolution-to-syscall window the 2026-09-04 resolve-then-open fix
// left open, found by the same day's red-probe round; fixed by routing
// every LocalStorage syscall through an *os.Root pinned to the resolved
// base, so a symlink planted after resolution is refused by the kernel
// at the syscall instead of followed.
//
...
// issuing the syscall is not containment: a symlink planted into the
// tree AFTER the resolution must still be refused — by the kernel, at
// the syscall, not by a check that already ran.
// Surfaces: core/upload/local.go:Save, ::open (serves Get and
// GetRange), ::Delete, ::Exists — all behind resolveKey, whose
...
// in parallel while it is installed).
...
func TestLocalStorageRefusesMidResolvePlant(t *testing.T) {
	...
	t.Run("Get", func(t *testing.T) {
		store := NewLocalStorage(base)
		install(func() {
			_ = os.Symlink(secret, filepath.Join(base, "victim.txt"))
		})
		defer os.Remove(filepath.Join(base, "victim.txt"))

		rc, err := store.Get(ctx, "victim.txt")
		if err == nil {
			body, _ := io.ReadAll(rc)
			rc.Close()
			if string(body) == "OUTSIDE-SECRET" {
				t.Fatalf("SECURITY: [upload-plant-read] Get served OUTSIDE-SECRET from a symlink planted after resolution: the containment proof ran on a clean tree and the open followed a tree that was swapped underneath it.")
			}
		}
	})
	...
	t.Run("GetRange", func(t *testing.T) {
		...
		rs, err := store.GetRange(ctx, "victim.txt")
		if err == nil {
			body, _ := io.ReadAll(rs)
			rs.Close()
			if string(body) == "OUTSIDE-SECRET" {
				t.Fatalf("SECURITY: [upload-plant-read] GetRange served OUTSIDE-SECRET from a post-resolution symlink plant.")
			}
		}
	})
	...
	t.Run("Exists", func(t *testing.T) {
		...
		if ok, err := store.Exists(ctx, "victim.txt"); err == nil && ok {
			t.Fatalf("SECURITY: [upload-plant-exists] Exists answered true for a file OUTSIDE the storage root reached through a post-resolution symlink plant: an existence oracle for arbitrary paths.")
		}
	})
	...
	t.Run("Delete", func(t *testing.T) {
		...
		_ = store.Delete(ctx, "sub/victim.txt")
		if _, err := os.Stat(filepath.Join(outsideSub, "victim.txt")); err != nil {
			t.Fatalf("SECURITY: [upload-plant-delete] Delete unlinked a file OUTSIDE the storage root: the parent directory was swapped for a symlink after resolution and the remove followed it.")
		}
	})
	...
	t.Run("Save", func(t *testing.T) {
		...
		_ = store.Save(ctx, "sub2/new.txt", strings.NewReader("ESCAPED"))
		if _, err := os.Stat(filepath.Join(outsideSub2, "new.txt")); err == nil {
			t.Fatalf("SECURITY: [upload-plant-write] Save wrote a file OUTSIDE the storage root: the parent directory was swapped for a symlink after resolution and the staging write + rename followed it.")
		}
	})
}
