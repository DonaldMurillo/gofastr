...
// prologue and the matching epilogue. The body then subtracts another 64
// bytes for the outgoing stack-argument area, keeping SP 16-byte aligned
// at the BL (asmcgocall hands us an aligned SP and 96 is a multiple of
// 16).
//
// Offsets into the callArgs block (R0 on entry), pinned by the
// compile-time checks in call_linux.go:
//
...
//
//	 0..63   outgoing stack arguments (stks[0..7]), one full 8-byte
//	         slot each, which is AAPCS64's stack-argument packing on
//	         Linux
//	64..71   (free)
