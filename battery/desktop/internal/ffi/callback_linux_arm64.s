...
// start routine, a GIO async completion) with the slot index in R12.
// Unlike the darwin sibling it is NOT restricted to threads parked in a
// runtime.cgocall: runtime.cgocallback borrows an M through needm when
// the calling thread is not one Go created, and _cgo_getstackbound
// (fakecgo_linux_arm64.s) then gives that M the thread's real g0 bounds.
// Either way cgocallback switches onto a goroutine stack, runs
...
TEXT ·callbackasm1(SB),NOSPLIT,$0-0
	...
	// prologue (cmd/internal/obj/arm64/obj7.go:582-605), whose
	// SUB $autosize, RSP, R20 scratch use destroys the incoming R20, a
	// C-callee-saved register. The body extends the frame manually
	// instead (the runtime's amd64 callbackasm1 does the same with
	// SUBQ), by 336 bytes on top of the 16 the prologue owns: 352
	// total, 16-byte aligned throughout.
	//
	...
	// d0 for signatures that return a float. A pthread start routine
	// reads x0 as its void* return, which is why the foreign-thread
	// test can assert on it.
	//
	// The darwin sibling records a "first callback on an M reads this
	// slot back as zero" anomaly from the abandoned syscall9/libcCall
	// entry path. The same two tests pin it here on the cgocall path:
	// ffi.TestFirstCallbackReturnOnFreshM makes the first trampoline
	// call on 16 brand-new Ms, and the qsort GC regression asserts all
	// ~50k comparator returns by checking the array came back sorted.
	MOVD	312(RSP), R0
	MOVD	R0, 8(RSP)
	FMOVD	8(RSP), F0
