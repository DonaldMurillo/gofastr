...
// password-only. Production must warn when 2FA runs on it.
func TestProdWarnsOnMemoryTwoFAStore(t *testing.T) {
	out := captureSlog(t, func() {
		mgr := auth.New(auth.AuthConfig{JWTSecret: "k"})
		mgr.Use(auth.NewCorePlugin())
	...
	if !strings.Contains(out, "in-memory 2FA store") {
		t.Fatalf("production Init with in-memory 2FA state must warn; got log: %q", out)
	}
}
