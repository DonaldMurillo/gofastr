...
// Property: the stored OAuth refresh token never regresses below a concurrently-persisted
// newer grant — RefreshOAuthToken's own doc comment (oauth_token_store.go:317-334) promises
// a grant stored between the first Get and the final Save survives.
