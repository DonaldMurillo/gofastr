...
// Pins: no password-equivalent secret in a plaintext SQL column — the
// magic-link token table stores sha256(token) and keys lookup on the
// digest (the APIToken grammar). Found by the 2026-09-06/07 adversarial
// round 5, phase 2 (family enumeration, tier T2). BREAKING: tokens
// minted before the upgrade stop redeeming; they are short-lived, so a
// lookup miss reads as normal expiry.
...
// Pins: a session bearer token is not stored as a plaintext SQL column
// (EntitySessionStore stores sha256(token) and keys every lookup on the
// digest). Maintainer answer (2026-09-07): yes, hash-then-lookup —
// auth.md:1477's "session store is trusted" carve-out does not bless a
// plaintext column against the READ-ONLY exposure class (dump, backup,
// replica, SELECT-granted role), the exact rationale the OAuth sealing
// contract gives for its own column. Found by the 2026-09-06/07
// adversarial round 5, phase 2. BREAKING: sessions minted before the
// upgrade stop authenticating (cookie value no longer matches the
// digest); users re-login.
// Pinned sibling: TestOAuthTokenStore_StoredOpaque
