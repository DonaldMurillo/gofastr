...
// CONTRACT-QUESTION: no doc promise exists — doc.go:18-21 shows an
// https:// example only, and the config field comment
// (password_reset.go:28-30) says just 'e.g. "https://app.example.com"'.
// The in-repo precedent for a config-supplied origin is strict mode's
// invalidSitemapBaseURL (framework/uihost/strict.go:578-596), which
// refuses any scheme that is not http/https. Delete this test if the
// maintainer decides BaseURL stays operator-trusted input.
