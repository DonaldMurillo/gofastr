...
idx, _ := semantic.Open(semantic.Options{
    ...
    Keyword:  semantic.NewMemoryKeyword(),       // optional, enables hybrid
    Path:     "~/.gofastr/semantic/myapp",       // optional, enables persistence
})
defer idx.Close()

idx.Add(ctx, semantic.Document{ID: "auth", Source: "auth.go",
    Text: "Auth middleware verifies sessions and JWTs."})

hits, _ := idx.Query(ctx, semantic.Query{
...
| File | What it does |
| --- | --- |
| `semantic.go` | Public types: `Document`, `Chunk`, `Hit`, `Query`, `Filter`, `Stats`. The `Embedder`, `Chunker`, `Store`, `KeywordBackend`, `Reranker` interfaces. `Open(Options) Index`. |
...
| `plugin.go` | `framework.Plugin` adapter that auto-mounts `/semantic/*` routes. |
...
`semantic.NewWatcher(idx, semantic.WatchOptions{...})` walks roots, applies include-exts (`.go`, `.md`, …), excludes well-known dirs (`.git`, `node_modules`, …), and polls every 2s by default. Replace-by-doc semantics in `Index.Add` mean a file edit re-chunks cleanly without leaving stale chunks behind.

## HTTP surface

| Method | Path | Body | Returns |
| --- | --- | --- | --- |
| POST | `/semantic/index` | `{"documents":[…]}` | 202 + `{"added": N}` |
| POST | `/semantic/query` | `Query` | `{"hits":[…]}` |
| GET  | `/semantic/stats` | — | `Stats` |
...
```bash
gofastr semantic index .                          # one-shot index of cwd
gofastr semantic watch ./src ./docs               # poll-watch until SIGINT
gofastr semantic query "auth middleware" -k 5 --hybrid
gofastr semantic stats
gofastr semantic clear                            # wipe local snapshot
```

When `GOFASTR_URL` is set, `query` and `stats` hit that server's `/semantic/*` routes instead of opening a local index.
