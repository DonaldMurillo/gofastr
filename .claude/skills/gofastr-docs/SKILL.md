---
name: gofastr-docs
description: Auto-loads when adding, changing, or removing any GoFastr feature or exported API. Encodes the doc topology (README + ARCHITECTURE + docs/*.md + agent-notes) and the change→doc mapping. Triggers on edits to framework/*.go, core/, battery/, cmd/, kiln/, core-ui/ — and on phrases like "add", "implement", "build", "refactor", "rename", "remove", "deprecate", "new feature", "new endpoint", "new field type", "API change", "expose", "wire up". Goal: docs ship in the same commit as the code, not "later".
---
...
| `docs/entity-declarations.md` | JSON + Go entity declaration reference. | New field type, new option in `EntityConfig`, new declaration loader, new validator. |
| `docs/query-dsl.md` | Query DSL parser surface. | New operator, new clause, new column type, change to DSL grammar. |
| `docs/migrations.md` | SQL-file migrate directives + CLI subcommands. | New directive, new CLI subcommand, dialect change. |
| `docs/search.md` | `battery/search` interface + backends. | New backend, interface change, new query option. |
| `docs/security.md` | Default middleware stack + security headers. | New default middleware, header change, new policy primitive. |
| `docs/widgets.md` | `core-ui/widget` builder API. | New widget, new preset, new theme hook. |
| `docs/agent-notes.md` | Append-only review log. | After any architecture/security review or large refactor. One entry per pass. |
| `docs/project-architecture-review.md` | Living review of risk & gap findings. | Only when you are running a fresh review pass — not casually. |
...
- If the new symbol is referenced from a JSON entity declaration:
  `docs/entity-declarations.md`

**You added a new entity field type** (e.g. `schema.JSON`, `schema.Money`):
- `docs/entity-declarations.md` — field type table
...
- Append exactly ONE entry to `docs/agent-notes.md` with date, scope,
  symptom, evidence, next-time note (follow the existing format)
- Only update `docs/project-architecture-review.md` if you ran a fresh
...
- **Stub doc disease.** `docs/search.md`, `docs/security.md`,
  `docs/migrations.md`, `docs/query-dsl.md` were each ~20 lines —
