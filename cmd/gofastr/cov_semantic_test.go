...
func TestRunEmbedHelpAndUnknown(t *testing.T) {
	covT_embedSandbox(t)
	out := covT_capStdout(t, func() { runSemantic(nil) })
	if !strings.Contains(out, "semantic search") {
		t.Fatalf("help: %s", out)
	}
	out = covT_capStdout(t, func() { runSemantic([]string{"help"}) })
	if !strings.Contains(out, "Subcommands") {
		t.Fatalf("help sub: %s", out)
	}
	code := covT_capExit(t, func() {
		covT_capStdout(t, func() { runSemantic([]string{"bogus"}) })
	})
	if code != 1 {
...
func TestEmbedIndexQueryStatsClear(t *testing.T) {
	...
	var indexOut string
	code := covT_capExit(t, func() {
		indexOut = covT_capStdout(t, func() { runSemantic([]string{"index", "."}) })
	})
	if code != -1 {
		t.Fatalf("embed index exited %d: %s", code, indexOut)
	}
	out := covT_capStdout(t, func() { runSemantic([]string{"query", "authentication", "-k", "3", "--hybrid"}) })
	if !strings.HasPrefix(strings.TrimSpace(out), "[") {
		t.Fatalf("query should print JSON array: %s", out)
	}
	covT_capStdout(t, func() { runSemantic([]string{"stats"}) })
	covT_capStdout(t, func() { runSemantic([]string{"clear"}) })
}
...
func TestEmbedQueryFlagParsing(t *testing.T) {
	covT_embedSandbox(t)
	covT_capStdout(t, func() { runSemantic([]string{"index", "."}) })
	// --mmr value
...
func TestChooseEmbedderOllama(t *testing.T) {
	...
	if _, ok := chooseEmbedder().(*semantic.OllamaEmbedder); !ok {
...
func TestLocalSnapshotDir(t *testing.T) {
	...
	if !strings.Contains(dir, ".gofastr") || !strings.Contains(dir, "semantic") {
...
func TestEmbedRemoteQueryAndStats(t *testing.T) {
	...
	out := covT_capStdout(t, func() { runSemantic([]string{"query", "x"}) })
	if !strings.Contains(out, "a.go") {
		t.Fatalf("remote query: %s", out)
	}
	out = covT_capStdout(t, func() { runSemantic([]string{"stats"}) })
...
func TestRemoteQueryHTTPError(t *testing.T) {
	...
	defer srv.Close()
	if _, err := remoteQuery(srv.URL, semantic.Query{Text: "x"}); err == nil {
		t.Fatal("expected HTTP error from remoteQuery")
	}
	if _, err := remoteGet(srv.URL + "/semantic/stats"); err == nil {
...
func TestPrintHits(t *testing.T) {
	out := covT_capStdout(t, func() {
		printHits([]semantic.Hit{{Chunk: semantic.Chunk{DocID: "x.go", Text: "hi"}, Score: 0.5}})
	})
