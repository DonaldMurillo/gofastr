...
func decodeBlocks(node *coreyaml.Node) ([]BlueprintBlock, error) {
	...
	for i, item := range list {
		...
		allowed := map[string]bool{"type": true, "kind": true, "text": true, "level": true, "class": true, "href": true, "entity": true, "fields": true, "limit": true, "empty_text": true, "mode": true, "search": true, "props": true, "children": true, "actions": true, "island": true, "widget": true}
...
// boots the generated binary, then asserts the real flow: marketing home
// renders, gated app routes redirect to login, and (with a seeded admin) the
// authenticated screens render.
func renderBlueprintE2ETest(bp Blueprint) string {
	...
	for _, s := range bp.Screens {
		if s.Access.Auth {
			gated = blueprintScreenRoutePath(s.Route)
			break
		}
	}
	...
	needsAuthClient := gated != "" && adminEmail != "" && adminPass != ""
	...
	b.WriteString(fmt.Sprintf("\tif h := e2eGet(t, base+\"/\"); !strings.Contains(h, %q) {\n\t\tt.Errorf(\"home page missing brand\")\n\t}\n", appName))
	if gated != "" {
		b.WriteString("\tnoRedir := &http.Client{CheckRedirect: func(*http.Request, []*http.Request) error { return http.ErrUseLastResponse }}\n")
		b.WriteString(fmt.Sprintf("\tif r, err := noRedir.Get(base + %q); err == nil {\n", gated))
		b.WriteString("\t\tdefer r.Body.Close()\n")
		b.WriteString(fmt.Sprintf("\t\tif r.StatusCode != http.StatusSeeOther { t.Errorf(\"gated %s = %%d, want 303 redirect to login\", r.StatusCode) }\n", gated))
		b.WriteString("\t}\n")
		if adminEmail != "" && adminPass != "" {
			b.WriteString("\tjar, _ := cookiejar.New(nil)\n")
			b.WriteString("\tclient := &http.Client{Jar: jar}\n")
			b.WriteString(fmt.Sprintf("\tif _, err := client.PostForm(base+\"/auth/login\", url.Values{\"email\": {%q}, \"password\": {%q}, \"next\": {%q}}); err != nil { t.Fatalf(\"login: %%v\", err) }\n", adminEmail, adminPass, gated))
			b.WriteString(fmt.Sprintf("\tif r, err := client.Get(base + %q); err == nil {\n", gated))
			b.WriteString("\t\tbody, _ := io.ReadAll(r.Body); r.Body.Close()\n")
			b.WriteString(fmt.Sprintf("\t\tif r.StatusCode != http.StatusOK { t.Errorf(\"authed %s = %%d, want 200\", r.StatusCode) }\n", gated))
			b.WriteString("\t\tif len(body) < 200 { t.Errorf(\"authed screen suspiciously short\") }\n")
			b.WriteString("\t}\n")
		}
	}
	b.WriteString("}\n\n")
	...
	b.WriteString("func e2eGet(t *testing.T, u string) string {\n\tt.Helper()\n\tr, err := http.Get(u)\n\tif err != nil { t.Fatalf(\"GET %s: %v\", u, err) }\n\tdefer r.Body.Close()\n\tb, _ := io.ReadAll(r.Body)\n\treturn string(b)\n}\n")
	return b.String()
}
...
func blueprintResourceRegistry(bp Blueprint) string {
	...
	for _, e := range names {
		...
		sb.WriteString(fmt.Sprintf("\t\tTitle: %q, Singular: %q, BasePath: %q,\n", toDisplayName(e), singularize(toDisplayName(e)), base[e]))
		...
		for _, f := range decl.Fields {
			...
			sb.WriteString(fmt.Sprintf("\t\t\t{Key: %q, Label: %q, Type: %q},\n", f.Name, humanizeFieldLabel(f.Name), f.Type))
		}
		sb.WriteString("\t\t},\n")
...
func screenNeedsCtx(screen BlueprintScreen) bool {
	...
	walk = func(blocks []BlueprintBlock, top bool) bool {
		for _, b := range blocks {
			// Any entity list/detail — at any nesting level — is server-rendered
			// via the resource engine, which needs the request ctx.
			if isEntityListBlock(b) || isEntityDetailBlock(b) {
				return true
			}
	...
	for _, screen := range bp.Screens {
		...
		scan = func(blocks []BlueprintBlock, top bool) {
			for _, block := range blocks {
				...
				if isEntityListBlock(block) || isEntityDetailBlock(block) {
