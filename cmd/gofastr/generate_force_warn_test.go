...
func TestForceRemovesRetiredGeneratedResourceTest(t *testing.T) {
	dir := t.TempDir()
	covT_chdir(t, dir)
	bp := filepath.Join(dir, "bp.yml")
	if err := os.WriteFile(bp, []byte(testBlueprintYAML()), 0o644); err != nil {
	...
	retired := filepath.Join(dir, "resource_test.go")
	if err := os.WriteFile(retired, []byte("// Code generated by gofastr. Owned — safe to edit.\npackage main\n"), 0o644); err != nil {
		t.Fatal(err)
	}

	covT_capStdout(t, func() { runGenerate([]string{"--from=" + bp, "--force"}) })
	if _, err := os.Stat(retired); !os.IsNotExist(err) {
		t.Fatalf("retired generated resource_test.go still exists after --force: %v", err)
	}
}
