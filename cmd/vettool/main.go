...
// recurred, each wrapped in allow.Guard so a site that is the shape on
// purpose can carry //gofastr:allow(<name>) <why>. allow.Names lists the
// same set for the contracts meta-rule; TestAllowNamesMatchRegistration
// keeps the two in step.
var repoAnalyzers = []*analysis.Analyzer{
	// Repo invariants, each born from a bug that recurred.
	allow.Guard(mapwriter.Analyzer),
	allow.Guard(unboundedbody.Analyzer),
	allow.Guard(errleak.Analyzer),
	allow.Guard(fieldtypeswitch.Analyzer),
	...
	allow.Guard(controlbytes.Analyzer),
	allow.Guard(callbackunderlock.Analyzer),
	allow.Guard(recovercallback.Analyzer),
	allow.Guard(laxcoerce.Analyzer),
	allow.Guard(rootwrite.Analyzer),
	allow.Guard(divlimit.Analyzer),
	allow.Guard(intwrap.Analyzer),
	allow.Guard(reflectset.Analyzer),
	allow.Guard(discardederr.Analyzer),
	...
	allow.Guard(hygiene.EmptyErrBranchAnalyzer),
	allow.Guard(hygiene.ClientTimeoutAnalyzer),
	allow.Guard(reqparamlimit.Analyzer),
	allow.Guard(discardmutator.Analyzer),
...
func main() {
	multichecker.Main(append(repoAnalyzers,
