...
Three features that turned out to share one piece of plumbing. Embeddable
...
while you edit it. API versioning makes `v1` and `v2` of an entity coexist,
which the docs had described for months and the registry had panicked on.
...
- **BREAKING: `Registry.Get` errors instead of guessing** when one entity name
  has several versions. Use `GetVersioned(name, version)`. `Registry.All`
  still returns one representative per name, so anything iterating it sees
  only the lex-first version.
