...
func TestLightboxParseDeeplinkReservedKey(t *testing.T) {
	...
	// The guarded spelling writes through a hoisted key with the guard
	// before it. A Map or null-proto map is equally secure.
	write := strings.Index(body, "out[k] =")
	if write < 0 {
		if strings.Contains(body, "out.set(") || strings.Contains(body, "Object.create(null)") {
			return // Map-keyed / proto-proof map: plain keys, no re-parenting
		}
