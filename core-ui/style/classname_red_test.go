...
// CONTRACT-QUESTION red: classes.go/extract.go doc comments promise no
// sanitization, and no in-repo production caller runs GenerateChunk over
// reflected HTML today — this asserts the identifier-slot contract the
// package family already pins for StyleSheet property names
// (TestStyleSheetRejectsNonIdentPropNames) and FontFaceCSS; if raw class
// names are deliberate, escape them in docs instead and delete this.
//
// RED TEST — open finding, 2026-09-06 adversarial pass (round 5; tests-only;
// no fix applied).
//
