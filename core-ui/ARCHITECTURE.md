...
holds registered behaviours too, owning their own `data-local-*` markers
and reaching this store only through the API above. Its one ask of
core-ui is the `data-fui-rpc-with` request/response hook seam in
`rpc.js`.
