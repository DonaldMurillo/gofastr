...
| `data-fui-disclosure` | Legacy spelling, read by the KERNEL only: the navigator closes such a `<details>` on a client-side navigation and on an in-panel anchor click (`html.Details{Disclosure: true}` emits it). The full disclosure behaviour — the aria-expanded mirror, Escape one level at a time with focus return, the navigate-close for `data-hui-disclosure` markup, the trap posture — belongs to `headless.Disclosure`'s `data-hui-disclosure` hook, bound by the registered `headless-disclosure` module (framework/headless). |
| `data-fui-disclosure-persist` | Kernel close-exemption for a `data-fui-disclosure` `<details>`: the navigator leaves it open across a client-side navigation. The headless anatomy's equivalent carries a storage key (`data-hui-disclosure-persist="<key>"`, session-restored by the registered `headless-disclosure` module). |
...
| `data-fui-pane-deeplink` | The legacy spelling of the pane deep-link declaration, still read by the kernel\'s stateful-params scan (nav.js `_statefulParams`) so a history move across a pane param from an old host stays stateful; no handler owns it and no component emits it anymore. |
