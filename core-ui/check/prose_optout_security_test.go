...
// Pins: a check-csp opt-out is live only when the directive starts its
// own comment — prose or documentation merely QUOTING the directive
// never exempts a file. Found 2026-09-06/07 adversarial round 5
// (fringe wave; tier T2), fixed the same round by anchoring
// hasCSPIgnoreDirective.
//
// This is the house grammar, stated and implemented at
...
//
// Surfaces: core-ui/check/noinlinescripts.go::hasCSPIgnoreDirective was a
// whole-file strings.Contains until 2026-09-07, so any file whose prose
// mentioned the directive silently opted out of BOTH gates:
// LintNoInlineStyles (+Recursive) and LintNoInlineScripts(+Recursive) /
// ScanInlineScriptsIn (the contracts entry), the cmd/check-csp build
// gate, and the contracts GOFASTR1804/1805 via
// framework/contracts/analyzers/rendering.go:791-794 — whose own comment
// at :793 ("continue // file opted out with //check-csp:ignore-file")
// was the in-tree demonstration: rendering.go silently exempted itself
// from the inline-script contract.
//
// Fix applied: hasCSPIgnoreDirective now matches only comment-initial
// directive lines (reCSPIgnoreDirective), the way suppress.go:35
// anchors its directives. Pinned siblings kept as guards:
// TestLintNoInlineStyles_HonorsIgnoreDirective and
// TestLintNoInlineScripts_HonorsIgnoreDirective (a genuine top-of-file
// directive still suppresses).
