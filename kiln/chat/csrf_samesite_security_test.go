...
// sfsReq names the browser inputs crossSite reads. Empty origin/sfs
...
// sfsDo drives the mounted CSRF test world (l.ServeHTTP, both the
...
// csrfPost documents.
func sfsDo(t *testing.T, l *live.Live, method, path, body string, rq sfsReq) *httptest.ResponseRecorder {
	t.Helper()
	if rq.host == "" {
...
// proposePlan seeds one pending destructive plan, the card a human
// is supposed to eyeball before approve_plan runs.
func proposePlan(t *testing.T, tools *protocol.Tools, id string) {
	t.Helper()
...
// Pins: a "same-site" Sec-Fetch-Site value is not proof of same-origin.
...
func TestCSRFSameSiteFallsThrough(t *testing.T) {
	...
	for _, leg := range legs {
		rec := sfsDo(t, l, http.MethodPost, leg.path,
			`{"plan_id":"`+leg.planID+`"}`,
			sfsReq{host: "localhost:8765", origin: "http://localhost:3000", sfs: "same-site"})
	...
	rec := sfsDo(t, l, http.MethodPost, "/kiln/tool/approve_plan", `{"plan_id":"ss-agent"}`, sfsReq{})
	...
	rec = sfsDo(t, l, http.MethodPost, "/kiln/tool/approve_plan", `{"plan_id":"ss-self"}`,
		sfsReq{origin: "http://localhost:8765", sfs: "same-origin"})
...
func TestCSRFWriteGuardHostPinned(t *testing.T) {
	...
	if rec := sfsDo(t, l, http.MethodGet, "/kiln/world", "",
		sfsReq{host: "evil.test:8765", origin: "http://evil.test:8765", sfs: "same-origin"}); rec.Code != http.StatusForbidden {
	...
	rec = sfsDo(t, l, http.MethodPost, "/kiln/tool/approve_plan", `{"plan_id":"hb-agent"}`, sfsReq{})
