...
// this test instead.
//
// Pins: on the ACP surface, every Destructive-marked tool obtains human
...
func TestAcpResetSessionHumanGate(t *testing.T) {
	tools := newTools(t)
	...
	// The prompt itself legitimately journals one chat_user entry; what
	// REJECT must prevent is the truncation to zero an ungated
	// reset_session performs. Counting only that: the seeded entry must
	// still be there.
	if after < before {
		t.Errorf("SECURITY: [kiln-acp-destructive-ungated] journal shrank from %d to %d entries across a prompt the user answered with REJECT — reset_session dispatched without consent and truncated the only durable state", before, after)
	}
