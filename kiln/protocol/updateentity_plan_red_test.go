...
// Property: the destructive-op plan gate (requirePlan doc: "the destructive-op
...
func TestUpdateEntityRedPlanGate(t *testing.T) {
	tools := newTools(t)
	...
	if len(ent.Fields) != 2 {
		t.Errorf("SECURITY: [updateentity-plan-gate] posts now has %d fields (%v) after an ungated update_entity — body was dropped with no plan, no needs_plan, and no plan consumption; delete_field removing the same single field would have been blocked pending an approved plan", len(ent.Fields), redFieldNames(ent.Fields))
	}
}

func redFieldNames(fs []world.Field) []string {
