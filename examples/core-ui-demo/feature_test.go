...
// TestFeature_AnimationTransitions verifies transition CSS is present in styles.
func TestFeature_AnimationTransitions(t *testing.T) {
	...
	// Verify transition/animation CSS was generated by the Go StyleSheet builder
	hasTransition := strings.Contains(pageHTML, "transition") || strings.Contains(pageHTML, "animation")
	if !hasTransition {
		t.Error("generated CSS should contain transition or animation declarations")
	}

	// Check for keyframe animations
	hasKeyframes := strings.Contains(pageHTML, "@keyframes")
	if !hasKeyframes {
