...
// /components/drawer has a drawer widget mounted, so the drawer
// module must load when the page is visited.
func TestE2E_RuntimeSplit_DrawerLoadsOnMarker(t *testing.T) {
	if testing.Short() {
		t.Skip("e2e: -short")
	}
	base := startE2EServer(t)
...
func TestE2E_RuntimeSplit_ToastModuleFailureShowsFallback(t *testing.T) {
	...
	if err := chromedp.Run(ctx,
		...
		// We eval fetch + then poke the runtime's RPC header dispatcher
		// to mimic what dispatchRPC does.
