...
// Pins: an order's money fields are server-derived from its line items,
...
//
// Maintainer answer (round 5): recompute/refuse — subtotal/total derive
// from order_items rows via the lifecycle hooks in order_totals.go.
...
// TestOrdersTotalServerDerived: a logged-in customer creates an order
...
func TestOrdersTotalServerDerived(t *testing.T) {
	if testing.Short() {
	...
	// ── Delete arm: removing the item must zero the derived totals (the
	// AfterDelete hook recomputes the parent from the pre-image). ──
	rows := orderItemRows(t, client, base, controlID)
	if len(rows) == 0 {
		t.Fatalf("setup broken: control order reports no items to delete")
	}
	itemID, _ := rows[0]["id"].(string)
	if itemID == "" {
		t.Fatalf("setup broken: line item carries no id: %v", rows[0])
	}
	if code, resp := e2eDo(t, client, "DELETE", base+"/api/order_items/"+itemID, ""); code != http.StatusOK && code != http.StatusNoContent {
		t.Fatalf("setup broken: DELETE /api/order_items/%s = %d; body=%.300s", itemID, code, resp)
	}
	stored, items := orderTotal(t, controlID)
	if stored.FloatString(2) != "0.00" || items.FloatString(2) != "0.00" {
		t.Errorf("SECURITY: [ecommerce-client-totals] after deleting its only item the order stored total %s with items summing %s, want 0.00/0.00 — the delete path must recompute the parent order's money from the remaining rows", stored.FloatString(2), items.FloatString(2))
	}
}

// orderItemRows returns the order's include=items rows for the delete arm.
func orderItemRows(t *testing.T, client *http.Client, base, orderID string) []map[string]any {
	t.Helper()
	gcode, gbody := e2eDo(t, client, "GET", base+"/api/orders/"+orderID+"?include=items", "")
	if gcode != http.StatusOK {
		t.Fatalf("setup broken: GET order %s = %d; body=%.300s", orderID, gcode, gbody)
	...
	if err := json.Unmarshal([]byte(gbody), &one); err != nil {
		t.Fatalf("setup broken: order items decode (%v): %.300s", err, gbody)
	}
	return one.Data.Items
}
