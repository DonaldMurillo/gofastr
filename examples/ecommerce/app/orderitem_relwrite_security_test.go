...
// Pins: an owner-scoped row's belongs_to relation is a write-side
...
//
// Fix (framework, round 5): crud's write path now runs checkBelongsToScope —
// a belongs_to FK must resolve under the target's own owner/tenant/read
// scopes, the resolver the read side already used. Pinned sibling read-half guard:
...
// e2eWaitReady); sibling test: orders_total_security_test.go (field
// spellings).
...
// TestOrderItemRelOwnTargetOnly: alice must only attach order_items to
...
func TestOrderItemRelOwnTargetOnly(t *testing.T) {
	if testing.Short() {
