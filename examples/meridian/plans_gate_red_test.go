...
// Property: mutating the shared cross-tenant plans catalog requires an elevated grant, not
// mere session possession. The docs' own doctrine (entity-declarations.md:132-136): "a
// session requirement is not row scoping... Without OwnerField, every authenticated user
// reads and can overwrite every other user's rows." The four sibling entities
// (customers/subscriptions/invoices/payments) all carry OwnerField; plans is the one
// entity in examples/meridian registered with CRUD+MCP exposure and neither OwnerField
// nor an Access policy. The API-token leg IS gated — a customers:* token is 403'd off
// POST /api/plans (apitoken_test.go:94-96) — but a plain browser session bypasses that
// gate entirely: sessions stay unscoped by design (app.go:369 RequireAPIScopes only
// binds bearer tokens).
// Surfaces: examples/meridian/entities/plans.go::registerPlans :260-268 (no OwnerField,
// no Access) and its gofastr.yml twin :85-110 (no owner_field, unlike customers :115);
// REST PUT/PATCH/POST/DELETE /api/plans*; MCP tools plans_create/plans_update/
// plans_delete for any authenticated session.
// Finding (verified live, 2026-09-06): fresh signup session PUT /api/plans/<pro>
// {"price":"0.01"} → 200, price 99→0.01; POST → 201; DELETE → 204; MCP plans_update
// renames the shared plan. Every paying customer's subscription pricing flows from this
// table.
// Fix direction: OwnerField is wrong for a shared catalog — gate writes on an Access
// policy (admin / plans:write grant) so the read stays world-readable for pricing pages
// while mutations need elevation; apply the same policy to the MCP exposure.
...
func TestPlansGateRedSessionWriteRefused(t *testing.T) {
	if testing.Short() {
...
func TestPlansGateRedMCPWriteRefused(t *testing.T) {
	if testing.Short() {
		t.Skip("builds + boots the binary")
	}
	_ = dotenv.LoadAndApply(framework.DefaultDotEnvPaths()...)
