...
key)` works the moment middleware attaches a locale. The framework's
own surfaces, however, still emit hardcoded English. A "French"
deployment today shows French strings only where the app calls `T`
itself; everything else stays English.
...
- **`framework/ui` default copy** — Pagination "Next" / "Previous",
  ValidationSummary headings, EmptyState placeholders, Banner /
  Toast / Modal defaults are all literal strings.
